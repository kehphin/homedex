      ...
      <div className="bg-base-100 rounded-lg overflow-hidden">
        <div className="p-6">
          {!subscriptions || subscriptions.length === 0 ? (
            <p className="text-center text-base-content opacity-70">You have no active subscriptions.</p>
          ) : (
                ...
                <tbody>
                  {subscriptions.map((subscription) => (
                    <tr key={subscription.id}>
                      ...
                      <td>${subscription.price.toFixed(2)}/{subscription.interval}</td>
                      <td>
                        {subscription.status === 'canceled' && subscription.endedAt? (
                          `Canceled on ${new Date(subscription.endedAt).toLocaleDateString()}`
                        ) : subscription.cancelAtPeriodEnd && subscription.status === 'active' ? (
                          `Ending on ${new Date(subscription.periodEnd).toLocaleDateString()}`
                        ) : subscription.status === 'active' ? (
                          `Renewing on ${new Date(subscription.periodEnd).toLocaleDateString()}`
                        ) : null}
                      </td>
                      <td>
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          subscription.status === 'canceled' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                        }`}>
                          {subscription.status}
                        </span>
                      </td>
