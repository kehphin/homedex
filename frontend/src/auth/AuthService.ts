...
export class AuthService {
  ...
  static async disconnectProviderAccount(providerId: string, accountUid: string): Promise<ProviderAccountsResponse> {
    return await request('DELETE', URLs.PROVIDERS, { provider: providerId, account: accountUid });
  }

  static async requestPasswordReset(email: string): Promise<StatusMessageResponse> {
  ...
  static async getEmailVerification(key: string): Promise<EmailVerificationInfo> {
  ...
  static async getAuthenticators(): Promise<AuthenticatorsResponse | RecoveryCodesResponse> {
  ...
  static async activateTOTPAuthenticator(code: string): Promise<TOTPAuthenticatorResponse> {
  ...
  static async markEmailAsPrimary(email: string): Promise<EmailAddressesResponse> {
    return await request('PATCH', URLs.EMAIL, { email, primary: true });
  }

  static async requestEmailVerification(email: string): Promise<StatusMessageResponse> {
  ...
  static async changePassword(data: PasswordResetData): Promise<StatusMessageResponse> {
  ...
  static async authenticateByToken(providerId: string, token: string, process: AuthProcessType = AuthProcess.LOGIN): Promise<AuthState> {
  ...
  static redirectToProvider(providerId: string, callbackURL: string, process: AuthProcessType = AuthProcess.LOGIN): void {
